...
const getInnerPanelClass = (teamCode, darkMode = false) => {
  ...
  return darkMode ? 'bg-[#1a3d1a]' : 'bg-[#f7f5f2]';
};
...
function getTeamConfettiColors(teamCode) {
  ...
  return colors.length >= 2 ? [...colors, '#ffffff'] : ['#5FBFD8','#2D7B2F','#9BC43A','#ffffff'];
}
...
export default function PaniniAlbumBRA2014() {
  ...
  const stickers = useMemo(() => {
    ...
    return Array.from({ length: stickerCount }, (_, i) => {
      const id = i + 1;
      let code, type, label, pair, isShield;

      if (section) {
        if (section.stickers) {
          const def = section.stickers[i];
          code = def.code; label = def.label; type = def.type; pair = def.pair || false;
          isShield = false;
        } else {
          code     = `${section.codePrefix}${section.codeStart + i}`;
          type     = section.type;
          label    = section.getLabel ? section.getLabel(id) : `${id}`;
          pair     = false;
          isShield = false;
        }
      } else {
        ...
        label    = id === 1 ? 'Escudo' : (id === 2 ? (playerNames[currentTeam]?.[2] || 'Equipo') : (playerNames[currentTeam]?.[id] || `Jugador ${id}`));
        pair     = false;
        isShield = id === 1;
      }
      ...
      return {
        id,
        code,
        visibleNumber: codeToNumber[code] ?? code,
  ...
  // Brillantes: 8 INTRO + 32 escudos = 40 total
  const introBrillanteCodes = ['PANINI','INTRO1','INTRO2','INTRO3','INTRO4','INTRO5','INTRO6','INTRO7'];
  const shieldCodes         = selectionTeams.map(t => `${t}1`);
  const brilliantCodes      = [...introBrillanteCodes, ...shieldCodes];
    ...
    for (const teamCode of teams) {
      ...
      if (section) {
        ...
        codes.forEach((code, i) => {
          ...
          else                   label = section.getLabel ? section.getLabel(i + 1) : `${i + 1}`;
        ...
        for (let id = 1; id <= albumConfig.teamStickerCount; id++) {
          entries.push({
            code:     `${teamCode}${id}`,
            label:    id === 1 ? 'Escudo' : (id === 2 ? (playerNames[teamCode]?.[2] || 'Equipo') : (playerNames[teamCode]?.[id] || `Jugador ${id}`)),
            team:     teamCode,
  ...
  const searchResults = useMemo(() => {
    ...
    // Búsqueda por "00" → PANINI
    if (q === '00') {
      const entry = searchIndex.find(e => e.code === 'PANINI');
      return entry ? [entry] : [];
    }

    // Búsqueda por número entero 1–639
    const num = parseInt(q, 10);
    if (!isNaN(num) && String(num) === q && num >= 1 && num <= 639) {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#0f2a0f] text-white' : 'bg-[#5FBFD8] text-slate-800'}`}>

      {/* ── HEADER ── */}
      <header className={`border-b shadow-sm sticky top-0 z-50 transition-colors duration-300 ${darkMode ? 'bg-[#1a3a1a] border-[#1a5a1a]' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-4 flex flex-row gap-2 justify-between items-center">
          <div className="min-w-0">
            ...
            <div className={`mt-1 sm:mt-2 h-2 sm:h-2.5 w-24 sm:w-56 rounded-full overflow-hidden ${darkMode ? 'bg-[#0d2a0d]' : 'bg-slate-200'}`}>
              <div
                className="h-full rounded-full bg-gradient-to-r from-cyan-500 via-green-400 to-cyan-600 transition-all"
          ...
          <div className="flex items-center gap-2 shrink-0 relative">
            {searchOpen && (
              <div className="relative flex items-center gap-1">
                <input
                  ...
                  className={`px-3 py-2 rounded-xl text-sm font-black border-2 w-32 sm:w-48 outline-none transition-all ${darkMode ? 'bg-[#0d2a0d] border-[#1a5a1a] text-white placeholder-slate-500' : 'bg-white border-slate-300 text-slate-800 placeholder-slate-400'}`}
                />
                ...
                {searchResults.length > 0 && (
                  <div className={`absolute top-full right-0 mt-1 w-72 max-w-[calc(100vw-1.5rem)] rounded-2xl shadow-2xl overflow-hidden z-[200] ${darkMode ? 'bg-[#1a3a1a] border border-[#2a5a2a]' : 'bg-white border border-slate-200'}`}>
                    {searchResults.map(entry => (
                      <button key={entry.code} onClick={() => handleSearchSelect(entry)}
                        className={`w-full px-4 py-2.5 text-left flex items-center gap-3 border-b last:border-b-0 transition-colors ${darkMode ? 'border-[#1a4a1a] hover:bg-[#1a4a1a] text-white' : 'border-slate-100 hover:bg-slate-50'}`}>
                        <span className="text-xl leading-none shrink-0">{entry.teamFlag}</span>
                        <div className="min-w-0">
                          <div className={`font-black text-xs uppercase ${darkMode ? 'text-cyan-400' : 'text-cyan-600'}`}>
            ...
            <button onClick={() => setCurrentView('home')}
              className={`px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-sm sm:text-base transition-colors duration-300 ${darkMode ? 'bg-white text-cyan-700' : 'bg-cyan-700 text-white'}`}>
              HOME
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        {currentView === 'home' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            ...
            <button onClick={() => setCurrentView('repetidas')} className={`rounded-3xl p-8 shadow-xl text-left active:scale-95 transition-colors duration-300 ${darkMode ? 'bg-[#1a3d1a] text-white' : 'bg-white'}`}>
              🔁<br/>
        ...
        {currentView === 'otros-proyectos' && (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-2xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#1a3d1a] text-white' : 'bg-white'}`}>
            ...
            <button
              ...
              className={`mt-6 px-6 py-3 rounded-2xl font-black transition-colors duration-300 ${darkMode ? 'bg-[#0d2a0d] text-white' : 'bg-gray-200 text-gray-800'}`}
            >← VOLVER</button>
          </div>
        )}
        ...
        {currentView === 'stats-selections' && (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-4xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#1a3d1a] text-white' : 'bg-white'}`}>
            ...
            <button onClick={() => { setCurrentView('home'); setShowStats(true); }}
              className="mt-6 bg-cyan-700 text-white px-6 py-3 rounded-2xl font-black">VOLVER</button>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {teams.map(team => (
              <button key={team}
                ...
                className={`rounded-2xl p-4 shadow font-black italic active:scale-95 transition-colors duration-300 flex items-center gap-2 ${darkMode ? 'bg-[#1a3d1a] text-white' : 'bg-white'}`}>
        ...
        {currentView === 'groups' && (
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at center, #009C3B, #5FBFD8, #2D7B2F, #9BC43A, #009C3B, #5FBFD8, #2D7B2F)' }}
          >
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <button onClick={() => { setCurrentTeamIndex(0); setCurrentView('album'); }}
                ...
                style={{ backgroundColor: '#9BC43A', color: '#0f2a0f' }}>
                INTRO
              </button>

              <button onClick={() => { setCurrentTeamIndex(teams.indexOf('ESTADIOS')); setCurrentView('album'); }}
                className="col-span-2 rounded-2xl p-4 font-black text-2xl sm:text-3xl active:scale-95 transition-transform"
                style={{ backgroundColor: '#0d3b1a', color: '#5FBFD8' }}>
                ESTADIOS
              </button>
        ...
        {currentView === 'groups' && (
          <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t shadow-lg transition-colors duration-300 ${darkMode ? 'bg-[#1a3a1a] border-[#1a5a1a]' : 'bg-white border-slate-200'}`}>
            <div className="flex">
              <button onClick={() => setCurrentView('home')}
                className={`flex-1 py-4 font-black italic text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#1a5a1a] text-white' : 'border-slate-200'}`}>HOME</button>
              <div className={`flex-1 border-r ${darkMode ? 'border-[#1a5a1a]' : 'border-slate-200'}`} />
              <button onClick={() => { setCurrentTeamIndex(0); setCurrentView('album'); }}
          ...
          <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t shadow-lg transition-colors duration-300 ${darkMode ? 'bg-[#1a3a1a] border-[#1a5a1a]' : 'bg-white border-slate-200'}`}>
            <div className="flex">
              <button onClick={() => currentTeam === teams[0] ? setCurrentView('groups') : prevTeam()}
                className={`flex-1 py-4 font-black italic text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#1a5a1a] text-white' : 'border-slate-200'}`}>← ANTERIOR</button>
              <button onClick={() => setCurrentView('teams')}
                className={`flex-1 py-4 font-black uppercase text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#1a5a1a] text-white' : 'border-slate-200'}`}>ÍNDICE</button>
      ...
      {showStats && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl w-full max-w-md transition-colors duration-300 ${darkMode ? 'bg-[#1a3d1a] text-white' : 'bg-white'}`}>
            ...
            <div className="space-y-3 font-black">
              ...
              <div>Brillantes: {brilliantCompletedCount} / 40</div>
...
function AlbumPage({ currentTeam, currentTeamInfo, stickers, stickerCount, currentTeamCompleted,
  ...
  const titleColor  = isDarkTeam || currentTeam === 'ESTADIOS'
    ...
    <div className={`rounded-3xl px-4 pt-4 pb-24 sm:px-8 sm:pt-8 sm:pb-8 shadow-xl ${getTeamGradientClass(currentTeam)}`}>
      ...
      <div className="hidden lg:flex justify-between items-center mb-8 gap-4">
        <button onClick={onPrev}
          className={`rounded-full px-6 py-3 shadow font-bold italic transition-colors duration-300 ${darkMode ? 'bg-[#1a3a1a] text-white border border-[#2a5a2a]' : 'bg-white text-black'}`}>
          ← ANTERIOR
        </button>
        <div className="text-center">
          <div className="flex items-center justify-center gap-3 flex-wrap">
            ...
            <button onClick={onIndex}
              className="bg-cyan-700 text-white px-4 py-2 rounded-2xl font-black uppercase text-lg sm:text-2xl leading-none">
              INDICE
            </button>
          </div>
          <div className={`mt-2 text-sm uppercase tracking-[0.25em] ${isDarkTeam || currentTeam === 'ESTADIOS' ? 'text-white/80' : 'text-slate-500'}`}>
            {currentTeamInfo.federation}
          </div>
          <div className={`mt-3 text-2xl font-black ${isDarkTeam || currentTeam === 'ESTADIOS' ? 'text-white' : 'text-cyan-700'}`}>
      ...
      {/* Inner panel */}
      <div className={`overflow-hidden rounded-[2rem] border-4 transition-colors duration-300 ${darkMode ? 'border-[#1a5a1a] bg-[#1a3d1a]' : 'border-slate-200 bg-white'} grid lg:grid-cols-2`}>
        {currentTeam === 'INTRO' ? (
          ...
          <EstadiosBRA2014Panel stickers={stickers} currentTeam={currentTeam}
            darkMode={darkMode} toggleSticker={toggleSticker} justPastedCode={justPastedCode} highlightCode={highlightCode} />
        ) : (
...
function INTROPanel({ stickers, currentTeam, currentTeamInfo, darkMode, toggleSticker, justPastedCode, highlightCode }) {
  ...
  return (
    <>
      {/* Mobile: grid de 4 columnas */}
      <div className={`lg:hidden p-3 col-span-2 ${bgClass}`}>
        <div className="text-xl font-black uppercase text-white mb-3">INTRO · BRASIL 2014</div>
      ...
      {/* Desktop izquierda */}
      <div className={`hidden lg:block p-8 border-r transition-colors duration-300 ${darkMode?'border-[#1a5a1a]':'border-slate-300'} ${bgClass}`}>
      ...
      <div className={`hidden lg:block p-8 ${bgClass}`}>
        <div className="grid grid-cols-4 gap-4 mt-[8rem]">
...
// ── Estadios BRA2014 Panel (pares de figuritas verticales) ────────────────────
function EstadiosBRA2014Panel({ stickers, currentTeam, darkMode, toggleSticker, justPastedCode, highlightCode }) {
  const bgClass = getInnerPanelClass(currentTeam, darkMode);

  // Agrupar en pares: [EST1,EST2], [EST3,EST4], ...
  const pairs = [];
  for (let i = 0; i < stickers.length; i += 2) {
    pairs.push([stickers[i], stickers[i + 1]]);
  }
  const half = Math.ceil(pairs.length / 2); // 6

  const renderPair = ([s1, s2]) => (
    <div key={s1.code} className="col-span-4">
      <div className={`text-[9px] sm:text-[10px] uppercase font-black tracking-widest mb-1 px-0.5 truncate ${darkMode ? 'text-cyan-300' : 'text-cyan-200'}`}>
        {s1.label}
      </div>
    ...
    <>
      ...
      <div className={`hidden lg:block p-8 border-r transition-colors duration-300 ${darkMode?'border-[#1a5a1a]':'border-slate-300'} ${bgClass}`}>
        ...
        <div className="grid grid-cols-4 gap-3">
          {pairs.slice(0, half).map(pair => renderPair(pair))}
        </div>
      </div>
      {/* Desktop derecha */}
      <div className={`hidden lg:block p-8 ${bgClass}`}>
        <div className="grid grid-cols-4 gap-3 mt-[4.5rem]">
          {pairs.slice(half).map(pair => renderPair(pair))}
        </div>
      </div>
    </>
  );
...
function TeamPanel({ stickers, currentTeam, currentTeamInfo, darkMode, toggleSticker, justPastedCode,
  ...
  const GroupBox = () => {
    ...
    return (
      <div className="border-2 rounded-2xl p-2 flex flex-col justify-center"
        style={darkMode ? {backgroundColor:'#0d2a0d',borderColor:'#2a5a2a'} : {backgroundColor:'rgba(255,255,255,0.6)',borderColor:'#cbd5e1'}}>
        <div className="font-black uppercase text-[11px] mb-1.5 tracking-widest text-center"
          style={{ color: darkMode ? '#5FBFD8' : grpColor }}>
          GRUPO {grpKey}
        </div>
  ...
  return (
    <>
      ...
      {/* Desktop izquierda */}
      <div className={`hidden lg:block p-3 sm:p-8 border-b lg:border-b-0 lg:border-r transition-colors duration-300 ${darkMode?'border-[#1a5a1a]':'border-slate-300'} ${bgClass}`}>
...
function Sticker({ sticker, onToggle, currentTeam, darkMode = false, justPasted = false, highlighted = false }) {
  ...
  const emptyBg     = darkMode ? 'bg-[#0d2a0d] border-slate-600' : 'bg-white border-slate-300';
  const completedBg = darkMode ? 'bg-green-900 border-green-500' : 'bg-green-100 border-green-500';

  // Estilo dorado para figuritas brillantes (INTRO + escudos)
  const brillanteStyle = isBrillante && !sticker.repeated ? {
    background: sticker.completed
      ? 'linear-gradient(135deg, #fbbf24, #f59e0b, #fde68a, #f59e0b, #fbbf24)'
      : 'linear-gradient(135deg, #fef9c3, #fde68a, #fef9c3, #fbbf24, #fef3c7)',
    borderColor: '#f59e0b',
  } : undefined;
  ...
  return (
    ...
    >
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        {/* Número visible — nunca el código interno */}
        <div className={`text-[9px] sm:text-xs uppercase break-all ${sticker.repeated ? repeatedCodeClass : sticker.completed ? 'text-black font-extrabold' : isBrillante ? 'text-yellow-700 font-black' : 'text-slate-400 font-black'}`}>
          #{sticker.visibleNumber}
        </div>
...
function getPlayerNameForCode(code) {
  ...
  if (team) {
    ...
    if (m) {
      ...
      if (id === 2) return playerNames[team]?.[2] || 'Equipo';
...
function CelebrationModal({ celebration, teamData, teamThemes, getThemeKey, getTeamConfettiColors, onClose }) {
  ...
  const gradientClass = isAlbum
    ? 'from-cyan-400 via-green-500 to-cyan-600'
    : theme?.gradient || 'from-cyan-500 to-green-600';

  const confettiColors = isAlbum
    ? ['#5FBFD8','#2D7B2F','#9BC43A','#ffffff','#facc15']
